...
| IA Variable                      | cpp code block | Note                                                               |
|----------------------------------+----------------+--------------------------------------------------------------------|
| Characteristic Dead              |                | #compute (multiply [[https://github.com/firelab/behave/blob/master/src/behave/surfaceFuelbedIntermediates.h#L92][void sumFractionOfTotalSurfaceAreaBySizeClass(const double areaWeightingFactorDeadOrLive[FuelConstants::MAX_PARTICLES],
const double savrDeadOrLive[FuelConstants::MAX_PARTICLES], double summedWeightingFactors[FuelConstants::MAX_PARTICLES]);]] by [[https://github.com/firelab/behave/blob/master/src/behave/surfaceFuelbedIntermediates.h#L110][weightedMoisture_[FuelConstants::MAX_LIFE_STATES];]]  ) |
| Characteristic Live              |                | #compute (multiply [[https://github.com/firelab/behave/blob/master/src/behave/surfaceFuelbedIntermediates.h#L92][void sumFractionOfTotalSurfaceAreaBySizeClass(const double areaWeightingFactorDeadOrLive[FuelConstants::MAX_PARTICLES],
const double savrDeadOrLive[FuelConstants::MAX_PARTICLES], double summedWeightingFactors[FuelConstants::MAX_PARTICLES]);]] by [[https://github.com/firelab/behave/blob/master/src/behave/surfaceFuelbedIntermediates.h#L110][weightedMoisture_[FuelConstants::MAX_LIFE_STATES];]]  )       |
...
| IA Variable              | cpp code block                                                                                                             | Note                                                                          |
|--------------------------+----------------------------------------------------------------------------------------------------------------------------+-------------------------------------------------------------------------------|
| Torching Trees           | [[https://github.com/firelab/behave/blob/master/src/behave/spot.cpp#L581-L584][double Spot::getMaxMountainousTerrainSpottingDistanceFromTorchingTrees(LengthUnits::LengthUnitsEnum spottingDistanceUnits)]] | #review make sure if topography is null, is the flat distance passed through? |
| Burning Pile             | [[https://github.com/firelab/behave/blob/master/src/behave/spot.cpp#L581-L584][double Spot::getMaxMountainousTerrainSpottingDistanceFromTorchingTrees(LengthUnits::LengthUnitsEnum spottingDistanceUnits)]] | #review make sure if topography is null, is the flat distance passed through? |
| Wind-Driven Surface Fire | [[https://github.com/firelab/behave/blob/master/src/behave/spot.cpp#L581-L584][double Spot::getMaxMountainousTerrainSpottingDistanceFromTorchingTrees(LengthUnits::LengthUnitsEnum spottingDistanceUnits)]] | #review make sure if topography is null, is the flat distance passed through? |
...
| IA Variable                        | cpp code block | Note                 |
|------------------------------------+----------------+----------------------|
| Fuel types are randomly            |                | #todos (need setter) |
| Fuel types are in vegetation bands |                | #todos (need setter) |

**** Custom Fuel Models

| IA Variable   | cpp code block | Note                                        |
|---------------+----------------+---------------------------------------------|
| English Units |                | #todos (do no see any setters in [[https://github.com/firelab/behave/blob/master/src/behave/surface.h][surface.h]]) |
| Metric Units  |                | #todos (do no see any setters in [[https://github.com/firelab/behave/blob/master/src/behave/surface.h][surface.h]]) |
...
| Moisture Scenario               |                             | #todos (could not find setters in [[https://github.com/firelab/behave/blob/master/src/behave/surface.h][surface.h]]) |

**** Dynamic Curing Load Transfer

| IA Variable                     | cpp code block | Note                                                              |
|---------------------------------+----------------+-------------------------------------------------------------------|
| Calculated from Live Herbaceous |                | #todos (could not find reference to dynamic curing load transfer) |
| Input Directly                  |                | #todos (could not find reference to dynamic curing load transfer) |
...
| IA Variable                           | cpp code block                                                                                     | Note |
|---------------------------------------+----------------------------------------------------------------------------------------------------+------|
| 20-ft Wind and Wind Adjustment Factor | [[https://github.com/firelab/behave/blob/master/src/behave/spotInputs.h#L80][setWindSpeedAtTwentyFeet(double windSpeedAtTwentyFeet, SpeedUnits::SpeedUnitsEnum windSpeedUnits);]] |      |
| Midflame (eye level) Wind             |                                                                                                    |      |
| 10-m Wind and Wind Adjustment Factor  |                                                                                                    |      |
| Wind and Slope                        |                                                                                                    |      |
|                                       |                                                                                                    |      |
...
| IA Variable                               | cpp code block | Note                                             |
|-------------------------------------------+----------------+--------------------------------------------------|
| are aligned wind is +- 30 from upslope    |                | #todos (no reference in [[https://github.com/firelab/behave/blob/master/src/behave/spotInputs.h][spotInputs.h]] or [[https://github.com/firelab/behave/blob/master/src/behave/spot.h][spot.h]] ) |
| are NOT aligned wind is > 30 from uplsope |                | #todos (no reference in [[https://github.com/firelab/behave/blob/master/src/behave/spotInputs.h][spotInputs.h]] or [[https://github.com/firelab/behave/blob/master/src/behave/spot.h][spot.h]] ) |
